...
class RbTree:
    ...
    def rb_delete(self, node):
        ...
        #self.root.size -= 1
        self.full_size -=1
        if self.root is not self.nil and self.root.key >= node.key:
            self.left_size -= 1
        else:
            self.right_size -= 1
    ...
    def rb_print(self):
        ...
        #self.Inorder_Tree_Walk(self.root)
            ...
            #print("!!!root: ",self.root, " leftSIZE: ", self.left_size,  " rightSize: ", self.right_size)
    ...
    def select(self, k):
        print(self.kth_smallest(self.root, k))
    ...
    #with open(input("Enter Filename: "), 'r') as f:
        ...
        for _ in range(n):
            ...
            if command == "insert":
                ...
                if t.rb_search(int(key[0])) is None:
                    print("Not Found")
                else:
                    print("Found")
            elif command == "min":
                if t.tree_minimum(t.root) is None:
                    print("Empty")
                else:
                    print(t.tree_minimum(t.root))
            elif command == "max":
                if t.tree_maximum(t.root) is None:
                    print("Empty")
                else:
                    print(t.tree_maximum(t.root))
            elif command == "select":
                t.select(int(key[0]))
